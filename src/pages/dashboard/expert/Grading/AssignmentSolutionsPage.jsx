...
export default function TeacherAssignmentSolutionsPage() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            try {
                ...
                const { data: res } = await gradingService.listAssignmentSolutions(params);
                const list = res?.data || {};
                setData(list.data || []);
                setMeta({
                ...
                setTotals({
                    ...
                    assignments_percentage: res?.assignments_percentage ?? 0,
                    assignments_status: res?.assignments_status ?? 'stable',
    ...
    const columns = useMemo(() => {
        ...
        return [
            ...
            },
            {
                ...
                Cell: (_, row) => row?.assignment?.title ?? '—',
            },
            {
                header: 'البرنامج',
                accessor: 'program_title',
                Cell: (_, row) => row?.assignment?.program?.title ?? '—',
            },
            {
                header: 'الحالة',
                accessor: 'status',
    ...
    return (
        <div className="p-3 lg:p-6 space-y-4 lg:space-y-6" dir="rtl">
            <h2 className="text-xl lg:text-2xl font-bold">تقييم المهام</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 lg:gap-4">
                <StatsCard
                    ...
                    trend={`${Math.round(totals.assignments_percentage)}%`}
                    up={totals.assignments_status === 'increase'}
                />
                <StatsCard
            ...
            {/* Filters + search (same UX as AccountsPage) */}
            ...
            <DataTable
                ...
                renderRowActions={(row) => (
                    <div className="flex gap-2">
                        <button
                            onClick={() => navigate(`/teacher/assessments/assignments/${row.id}`)}
